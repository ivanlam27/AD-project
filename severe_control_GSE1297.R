...
gse <- ReadAffy(celfile.path = "GSE1297_NEW")
series_matrix <- getGEO(filename = "GSE1297_series_matrix.txt/GSE1297_series_matrix.txt")
metadata <- series_matrix@phenoData@data
metadata <- metadata[c(1:1, 4:6, 10:11, 27, 13, 15:19, 24, 29:30), ]
...
#PCA PLOT
#colour <- c(rep('severe', 7), rep('incipient', 6), rep('moderate', 8), rep('control', 9))
#colour <- c(rep('Severe 701', 1), rep('Severe 723', 1), rep('Severe 807', 1), rep('Severe 819', 1), rep('Severe 867', 1), rep('Severe 872', 1), rep('Control 976', 1), rep('Control 1003', 1), rep('Control 1008', 1), rep('Control 1012', 1), rep('Control 1015', 1), rep('Control 1018', 1), rep('Control 1030', 1), rep('Control 1039', 1), rep('Control 1040', 1))
colour <- c(rep('Severe', 7), rep('Control', 9))
rawGSE <- exprs(gse)

#change scale and center to TRUE
pca_OG <- prcomp(rawGSE, scale=T, center=T)
