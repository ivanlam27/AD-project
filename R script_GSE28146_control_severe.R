...
gse <- ReadAffy(celfile.path = "GSE28146_RAW")
gse28146 <- getGEO(filename = "GSE28146_series_matrix.txt")
metadata <- gse28146@phenoData@data
CN <- metadata[c("title")]
...
#Raw Data Plot
colour <- c(rep('control',8), rep('severe', 7))
...
PCA_raw <- ggplot(pca_2, aes(x=PC1, y=PC2, color=colour))+ geom_point() + stat_ellipse() + labs(title = "Raw Data PCA Plot")
...
sampletype <- factor(c(rep("control",8), rep("severe",7)))
sampletype
matrix <- model.matrix(~factor(sampletype,levels = c('control','severe')))
fit_data <- lmFit(geneFilt,matrix)
efit <- eBayes(fit_data)
genes=geneNames(gse)
...
volcano_plot <-EnhancedVolcano(limma_output,
                               lab = rownames(limma_output),
                               pCutoff = 0.05,
                               FCcutoff = 0.5,
                               x = "logFC",
...
heatmap <- pheatmap(filt_50, main="Top 50 DEG", 
                         ...
                         annotation_colors=list(Tissue=c(AD="red", Control="blue")))
